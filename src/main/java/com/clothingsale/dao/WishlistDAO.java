...
public class WishlistDAO {
    ...
    private static final String SELECT_BY_USER_SQL
            = "SELECT w.user_id, w.product_id, w.variant_id, "
            + "w.created_at, w.updated_at, "
            + "p.product_name, p.slug, "
            + "p.status AS product_status, "
            + "COALESCE(variant_img.image_url, "
            + "product_img.image_url) AS main_image_url, "
            + "pv.sku, pv.sale_price, pv.stock_quantity, "
            ...
            + "     OR (pi.color IS NULL AND pi.variant_id = w.variant_id)) "
