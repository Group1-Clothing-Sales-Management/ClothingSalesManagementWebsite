...
<head>
    ...
    <style>
        ...
        .table thead th { background: #5c6bc0; color: #fff; font-weight: 600; font-size: .85rem; border: none; white-space: nowrap; }
        ...
        <c:if test="${empty pageMode or pageMode eq 'list'}">
            ...
            <div class="card card-main">
                <div class="card-header d-flex align-items-center justify-content-between flex-wrap gap-2">
                    ...
                    <form method="get" action="${pageContext.request.contextPath}/staff/customers" class="d-flex search-group gap-2">
                        <input type="hidden" name="action" value="list"/>
                        <div class="input-group input-group-sm">
                            <span class="input-group-text bg-white"><i class="bi bi-search text-muted"></i></span>
                            <input type="text" class="form-control" name="keyword" placeholder="Name, email, phone number..." value="${not empty keyword ? keyword : ''}"/>
                        ...
                        <c:when test="${empty customers}">
                            <div class="empty-state">
                                ...
                                <p class="text-muted small mt-1">
                                    <c:choose>
                                        <c:when test="${not empty keyword}">Try a different search keyword.</c:when>
                                        <c:otherwise>No customer records have been added yet.</c:otherwise>
                                    </c:choose>
                                </p>
                            </div>
                        </c:when>
                                    ...
                                    <tbody>
                                        <c:forEach var="c" items="${customers}" varStatus="st">
                                            <tr>
                                                ...
                                                <td>
                                                    <c:choose>
                                                        <c:when test="${c.status eq 'ACTIVE'}"><span class="badge badge-active px-2 py-1 rounded-pill">Active</span></c:when>
                                                        <c:when test="${c.status eq 'INACTIVE'}"><span class="badge badge-inactive px-2 py-1 rounded-pill">Inactive</span></c:when>
                                                        <c:otherwise><span class="badge badge-locked px-2 py-1 rounded-pill">Locked</span></c:otherwise>
                                                    </c:choose>
                                                </td>
                                                <td>
                    ...
                    <form method="post" action="${pageContext.request.contextPath}/staff/customers" novalidate>
                        ...
                        <div class="row g-3 mb-4">
                            <div class="col-md-6">
                                ...
                                <input type="text" name="username" class="form-control ${not empty errors.username ? 'is-invalid' : ''}" value="${formData.username}" placeholder="e.g. johndoe"/>
                        ...
                        <div class="row g-3">
                            <div class="col-md-6">
                                ...
                                <input type="text" name="fullName" class="form-control ${not empty errors.fullName ? 'is-invalid' : ''}" value="${formData.fullName}" placeholder="John Doe"/>
                                <c:if test="${not empty errors.fullName}"><div class="invalid-feedback">${errors.fullName}</div></c:if>
                            </div>
                            <div class="col-md-6">
                                <label class="form-label">Phone number <span class="required-star">*</span></label>
                                <input type="text" name="phone" class="form-control ${not empty errors.phone ? 'is-invalid' : ''}" value="${formData.phone}" placeholder="09xxxxxxxx"/>
                                <c:if test="${not empty errors.phone}"><div class="invalid-feedback">${errors.phone}</div></c:if>
                            </div>
                            <div class="col-12">
                                <label class="form-label">Email <span class="required-star">*</span></label>
                                <input type="email" name="email" class="form-control ${not empty errors.email ? 'is-invalid' : ''}" value="${formData.email}" placeholder="example@email.com"/>
                    ...
                    <form method="post" action="${pageContext.request.contextPath}/staff/customers" novalidate>
                        ...
                        <div class="row g-3 mb-4">
                            <div class="col-md-6">
                                ...
                                <input type="text" class="form-control bg-light" value="${customer.username}" disabled/>
                                <div class="form-text">The username cannot be changed.</div>
                            </div>
                            <div class="col-md-6">
                                <label class="form-label">Status <span class="required-star">*</span></label>
                                <select name="status" class="form-select ${not empty errors.status ? 'is-invalid' : ''}">
                                ...
                                <c:if test="${not empty errors.status}"><div class="invalid-feedback">${errors.status}</div></c:if>
                            </div>
                        </div>
                        ...
                        <div class="row g-3">
                            <div class="col-md-6">
                                <label class="form-label">Full name <span class="required-star">*</span></label>
                                <input type="text" name="fullName" class="form-control ${not empty errors.fullName ? 'is-invalid' : ''}" value="${customer.fullName}"/>
                                <c:if test="${not empty errors.fullName}"><div class="invalid-feedback">${errors.fullName}</div></c:if>
                            </div>
                            <div class="col-md-6">
                                <label class="form-label">Phone number <span class="required-star">*</span></label>
                                <input type="text" name="phone" class="form-control ${not empty errors.phone ? 'is-invalid' : ''}" value="${customer.phone}"/>
                                <c:if test="${not empty errors.phone}"><div class="invalid-feedback">${errors.phone}</div></c:if>
                            </div>
                            <div class="col-12">
                                <label class="form-label">Email <span class="required-star">*</span></label>
                                <input type="email" name="email" class="form-control ${not empty errors.email ? 'is-invalid' : ''}" value="${customer.email}"/>
                                <c:if test="${not empty errors.email}"><div class="invalid-feedback">${errors.email}</div></c:if>
                            </div>
                        </div>
                        ...
                        <div class="row g-3 mb-2">
                            <div class="col-md-6">
                                ...
                                <input type="text" class="form-control bg-light text-muted" value="${customer.createdAt}" disabled/>
                            </div>
                            <div class="col-md-6">
                                <label class="form-label text-muted">Last updated</label>
                                <input type="text" class="form-control bg-light text-muted" value="${customer.updatedAt}" disabled/>
                            </div>
                        </div>
