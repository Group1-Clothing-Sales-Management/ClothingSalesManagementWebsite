            ...
            <c:choose><c:when test="${empty returnRequest}"><div class="alert alert-danger">Return request not found.</div></c:when><c:otherwise>
                <div class="row g-4">
                    <div class="col-lg-8"><div class="card card-main mb-4"><div class="card-body p-4">
                        ...
                        <div class="row g-3 mb-4"><div class="col-md-4"><div class="text-muted small">Customer</div><strong>${returnRequest.customerName}</strong></div><div class="col-md-4"><div class="text-muted small">Type</div><strong>Return &amp; refund</strong></div><div class="col-md-4"><div class="text-muted small">Reason</div><strong>${returnRequest.reason}</strong></div></div>
                        <div class="d-flex justify-content-between align-items-center flex-wrap gap-2 mt-4 mb-2"><div><h3 class="h6 fw-bold mb-1">Returned product inspection</h3><p class="small text-muted mb-0">Check each product after physically inspecting it. Stock is updated only after every row is checked.</p></div><span class="badge text-bg-${returnRequest.inspectionComplete ? 'success' : 'warning'}">${returnRequest.inspectedItemCount}/${fn:length(returnRequest.items)} inspected</span></div>
                        <div class="table-responsive border rounded"><table class="table align-middle mb-0"><thead><tr><th>Product</th><th>Variant</th><th>Quantity</th><th>Unit price</th><th>Current stock</th><th>Inspection</th></tr></thead><tbody><c:forEach var="item" items="${returnRequest.items}"><tr><td>${item.productNameSnapshot}</td><td>${item.variantAttributesSnapshot}</td><td>${item.quantity}</td><td><fmt:formatNumber value="${item.unitPrice}" pattern="#,##0"/> ₫</td><td>${item.currentStock}</td><td><c:choose><c:when test="${returnRequest.status eq 'APPROVED'}"><form method="post" action="${returnsBasePath}" class="m-0"><input type="hidden" name="action" value="inspectItem"><input type="hidden" name="id" value="${returnRequest.id}"><input type="hidden" name="itemId" value="${item.id}"><input type="hidden" name="returnMode" value="detail"><input type="hidden" name="inspected" value="false"><label class="form-check mb-0"><input class="form-check-input" type="checkbox" ${item.inspected ? 'checked' : ''} onchange="this.form.inspected.value=this.checked; this.form.submit();"><span class="form-check-label">${item.inspected ? 'Checked' : 'Check after inspection'}</span></label></form></c:when><c:when test="${item.inspected}"><span class="text-success fw-semibold"><i class="fa-solid fa-circle-check me-1"></i>Checked</span></c:when><c:otherwise><span class="text-muted">Not checked</span></c:otherwise></c:choose></td></tr></c:forEach></tbody></table></div>
                        ...
                        <div class="mt-3"><div class="text-muted small">REFUND AMOUNT</div><div class="fs-4 fw-bold text-primary"><fmt:formatNumber value="${returnRequest.refundAmount}" pattern="#,##0"/> ₫</div></div>
                    ...
                    <div class="col-lg-4"><div class="card card-main return-actions"><div class="card-body p-4"><h3 class="h5 fw-bold mb-3">Available actions</h3>
                        ...
                        <c:if test="${returnRequest.status eq 'APPROVED'}"><c:if test="${not returnRequest.inspectionComplete}"><div class="alert alert-warning small">Inspect all returned products before confirming receipt.</div></c:if><form method="post" action="${returnsBasePath}"><input type="hidden" name="action" value="receive"><input type="hidden" name="id" value="${returnRequest.id}"><input type="hidden" name="returnMode" value="detail"><textarea name="note" class="form-control mb-2" rows="2" placeholder="Condition on arrival (optional)"></textarea><button class="btn btn-primary w-100" type="submit" ${returnRequest.inspectionComplete ? '' : 'disabled'}>Confirm products received</button></form></c:if>
