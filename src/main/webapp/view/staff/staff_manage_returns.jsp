...
<div class="admin-page">
    ...
    <c:choose>
        <c:when test="${pageMode eq 'detail'}">
            <div class="page-header"><div><h1 class="page-title"><i class="fa-solid fa-rotate-left"></i>Return Request Details</h1><p class="subtext mt-1">Review the request, record returned products, and follow the refund workflow.</p></div><a class="btn btn-outline-secondary" href="${returnsBasePath}">Back to requests</a></div>
            <c:choose><c:when test="${empty returnRequest}"><div class="alert alert-danger">Return request not found.</div></c:when><c:otherwise>
                <div class="row g-4">
                    <div class="col-lg-8"><div class="card card-main mb-4"><div class="card-body p-4">
                        <div class="return-detail-header d-flex justify-content-between align-items-start gap-3 mb-3"><div><div class="text-muted small">REQUEST CODE</div><h2 class="h4 fw-bold mb-1">${returnRequest.requestCode}</h2><div class="text-muted">Order ${returnRequest.orderCode}</div></div><span class="return-status return-status--${fn:toLowerCase(fn:replace(returnRequest.status, '_', '-'))}">${returnRequest.status}</span></div>
                    ...
                    <div class="col-lg-4"><div class="card card-main return-actions"><div class="card-body p-4"><h3 class="h5 fw-bold mb-3">Available actions</h3>
                        ...
                        <%-- Sau khi nhận hàng, Staff lập đề nghị để Admin duyệt tiền. --%>
                        <c:if test="${returnRequest.status eq 'RECEIVED'}"><form method="post" action="${returnsBasePath}"><input type="hidden" name="action" value="requestRefund"><input type="hidden" name="id" value="${returnRequest.id}"><input type="hidden" name="returnMode" value="detail"><button class="btn btn-primary w-100" type="submit">Submit refund for approval</button></form></c:if>
                        ...
                        <c:if test="${returnRequest.status eq 'COMPLETED' or returnRequest.status eq 'REJECTED'}"><div class="alert alert-light border mb-0">No more actions are available for this request.</div></c:if>
                    </div></div></div>
                </div>
