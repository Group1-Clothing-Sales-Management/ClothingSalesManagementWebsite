    ...
    <c:choose>
        ...
        <c:otherwise>
            ...
            <section class="detail-hero">
                ...
                <div class="detail-hero__amount"><c:choose><c:when test="${returnRequest.requestType eq 'RETURN'}"><span class="detail-hero__amount-label">Estimated refund</span><strong><fmt:formatNumber value="${returnRequest.refundAmount}" pattern="#,##0"/> VND</strong><small><i class="fa-solid fa-shield-heart me-1"></i>Processed securely</small></c:when><c:otherwise><span class="detail-hero__amount-label">Request type</span><strong>Exchange</strong><small><i class="fa-solid fa-arrows-rotate me-1"></i>No refund required</small></c:otherwise></c:choose></div>
            </section>
            ...
            <section class="detail-card detail-section" aria-label="Return progress">
                <div class="return-timeline">
                    ...
                    <div class="timeline-step ${returnStep >= 4 ? 'is-done' : ''} ${returnStep eq 4 ? 'is-current' : ''}"><span class="timeline-step__dot"><c:choose><c:when test="${returnStep >= 4}"><i class="fa-solid fa-check"></i></c:when><c:otherwise>4</c:otherwise></c:choose></span><span><c:choose><c:when test="${returnRequest.requestType eq 'RETURN'}">Refund complete</c:when><c:otherwise>Exchange complete</c:otherwise></c:choose></span></div>
                </div>
            </section>
            ...
            <div class="detail-grid">
                <div>
                    ...
                    <c:if test="${returnRequest.status eq 'COMPLETED' and returnRequest.requestType eq 'EXCHANGE'}"><div class="state-message state-message--green"><i class="fa-solid fa-arrows-rotate"></i><div><h2>Exchange completed</h2><p>The returned products were inspected and added back to stock. Your exchange is complete.</p></div></div></c:if>
