                    ...
                    <main class="container orders-page">
                        ...
                        <div class="order-search">
                            ...
                            <input id="orderSearch" type="search"
                                ...
                                aria-label="Search orders by seller name, order ID or product name" autocomplete="off">
                        </div>
                        ...
                        <section class="orders-list" id="ordersList">
                            <c:forEach items="${orders}" var="o">
                                ...
                                <c:set var="statusClass"
                                    value="${not empty o.displayStatusBadgeClass ? o.displayStatusBadgeClass : 'status-default'}" />
                                <c:if
                                    test="${o.orderStatus eq 'PENDING' or o.displayStatus eq 'PENDING' or o.displayStatus eq 'PENDING_APPROVAL'}">
                                    <c:set var="statusClass" value="status-pending" />
                                </c:if>
                                <c:if
                                    test="${o.orderStatus eq 'CONFIRMED' or o.displayStatus eq 'CONFIRMED' or o.displayStatus eq 'APPROVED'}">
                                ...
                                <c:if
                                    test="${o.orderStatus eq 'DELIVERED' or o.orderStatus eq 'SUCCESS' or o.displayStatus eq 'DELIVERED' or o.displayStatus eq 'SUCCESS' or o.displayStatus eq 'RECEIVED' or o.shippingStatus eq 'DELIVERED' or o.shippingStatus eq 'SUCCESS'}">
                                    <c:set var="statusClass" value="status-delivered" />
                                </c:if>
                                <c:if
                                    test="${o.orderStatus eq 'COMPLETED' or o.orderStatus eq 'SUCCESS' or o.displayStatus eq 'COMPLETED'}">
                                ...
                                <c:if
                                    test="${o.orderStatus eq 'CANCELLED' or o.displayStatus eq 'CANCELLED' or o.shippingStatus eq 'CANCELLED'}">
                                    <c:set var="statusClass" value="status-cancelled" />
                                </c:if>
                                <c:if
                                    test="${o.orderStatus eq 'RETURNED' or o.displayStatus eq 'RETURNED' or o.shippingStatus eq 'RETURNED'}">
                                    <c:set var="statusClass" value="status-returned" />
                                </c:if>
                                <c:set var="canBuyAgain"
                                    value="${o.displayStatus eq 'COMPLETED' or o.displayStatus eq 'CANCELLED' or o.orderStatus eq 'COMPLETED' or o.orderStatus eq 'SUCCESS' or o.orderStatus eq 'CANCELLED' or o.orderStatus eq 'FAILED' or o.shippingStatus eq 'CANCELLED' or o.shippingStatus eq 'FAILED'}" />
                                ...
                                <article class="order-card" data-order-search>
                                    <header class="shop-header">
                                        ...
                                        <div class="shop-header-right">
                                            <span
                                                class="shop-status ${statusClass eq 'status-completed' or statusClass eq 'status-delivered' or statusClass eq 'status-paid' ? 'delivery-success' : ''}">
                                                <c:choose>
                                                    <c:when
                                                        test="${statusClass eq 'status-completed' or statusClass eq 'status-delivered' or statusClass eq 'status-paid'}">
                                                        <i class="fa-solid fa-truck"></i> Delivery successful
                                                    </c:when>
                                                    <c:otherwise>
                                                        <i class="fa-solid fa-truck"></i>
                                                        <c:out
                                                            value="${not empty o.shippingStatusLabel ? o.shippingStatusLabel : 'Order status'}" />
                                                    </c:otherwise>
                                                </c:choose>
                                                <span class="status-divider">|</span>
                                                <span class="status-value ${statusClass}">
                                                    <c:out
                                                        value="${not empty o.displayStatusLabel ? o.displayStatusLabel : o.orderStatus}" />
                                                </span>
                                            </span>
                                    ...
                                    <div class="order-items">
                                        <c:forEach items="${o.details}" var="d">
                                            <div class="order-item">
                                                <c:set var="orderImageUrl"
                                                    value="${fn:replace(fn:trim(d.currentImageUrl), '/uploads/product/', '/media/product/')}" />
                                                <c:set var="orderImageUrl"
                                                    value="${fn:replace(orderImageUrl, 'uploads/product/', 'media/product/')}" />
                                                <c:set var="contextPathPrefix"
                                                    value="${pageContext.request.contextPath}/" />
                                                <c:choose>
                                                    <c:when test="${not empty orderImageUrl}">
                                                        <c:choose>
                                                            <c:when
                                                                test="${fn:startsWith(orderImageUrl, 'http://') or fn:startsWith(orderImageUrl, 'https://')}">
                                                                <img class="order-item-img" src="${orderImageUrl}"
                                                                    alt="${d.productNameSnapshot}"
                                                                    onerror="this.onerror=null;this.style.visibility='hidden';">
                                                            </c:when>
                                                            <c:when
                                                                test="${fn:startsWith(orderImageUrl, contextPathPrefix)}">
                                                                <img class="order-item-img" src="${orderImageUrl}"
                                                                    alt="${d.productNameSnapshot}"
                                                                    onerror="this.onerror=null;this.style.visibility='hidden';">
                                                            </c:when>
                                                            <c:when test="${fn:startsWith(orderImageUrl, '/')}">
                                                                <img class="order-item-img"
                                                                    src="${pageContext.request.contextPath}${orderImageUrl}"
                                                                    alt="${d.productNameSnapshot}"
                                                                    onerror="this.onerror=null;this.style.visibility='hidden';">
                                                            </c:when>
                                                            <c:when
                                                                test="${fn:startsWith(orderImageUrl, 'media/product/')}">
                                                                <img class="order-item-img"
                                                                    src="${pageContext.request.contextPath}/${orderImageUrl}"
                                                                    alt="${d.productNameSnapshot}"
                                                                    onerror="this.onerror=null;this.style.visibility='hidden';">
                                                            ...
                                                            <c:when test="${fn:contains(orderImageUrl, '/')}">
                                                                <img class="order-item-img"
                                                                    src="${pageContext.request.contextPath}/uploads/${orderImageUrl}"
                                                                    alt="${d.productNameSnapshot}"
                                                                    onerror="this.onerror=null;this.style.visibility='hidden';">
                                                            </c:when>
                                                            <c:otherwise>
                                                                <img class="order-item-img"
                                                                    src="${pageContext.request.contextPath}/media/product/${orderImageUrl}"
                                                                    alt="${d.productNameSnapshot}"
                                                                    onerror="this.onerror=null;this.style.visibility='hidden';">
                                                            </c:otherwise>
                                                        </c:choose>
                                                    ...
                                                    <c:otherwise>
                                                        <span class="order-item-img order-item-img-placeholder"
                                                            role="img" aria-label="${d.productNameSnapshot}"></span>
                                                    </c:otherwise>
                                                </c:choose>
                                    ...
                                    <footer class="order-footer">
                                        <c:if
                                            test="${o.voucherId > 0 and o.discountAmount != null and o.discountAmount > 0}">
                                            <div class="order-used-voucher">
                                                <span class="order-used-voucher-label">
                                                    ...
                                                    <span class="order-used-voucher-code">
                                                        <c:out
                                                            value="${not empty o.voucherCode ? o.voucherCode : 'Voucher'}" />
                                                    </span>
                                                </span>
                                                <c:if test="${not empty o.voucherTitle}">
                                                    <span>
                                                        <c:out value="${o.voucherTitle}" />
                                                    </span>
                                                </c:if>
                                        ...
                                        <div class="order-actions">
                                            <c:if test="${o.orderStatus eq 'PENDING'}">
                                                <form method="post"
                                                    ...
                                                    <button class="order-action danger" type="submit">Cancel
                                                        Order</button>
                                                </form>
                                            </c:if>
                                            ...
                                            <c:if test="${canBuyAgain}">
                                                <form method="post"
                                                    ...
                                                    <input type="hidden" name="orderId" value="${o.id}">
                                                    <button class="order-action primary" type="submit">Buy
                                                        Again</button>
                                                </form>
                                            </c:if>

                                            <%-- Chỉ hiện nút này cho đơn đã giao; controller vẫn kiểm tra lại thời hạn
                                                và quyền sở hữu. --%>
                                                <c:if
                                                    test="${o.orderStatus eq 'DELIVERED' or o.orderStatus eq 'SUCCESS' or o.orderStatus eq 'COMPLETED' or o.displayStatus eq 'DELIVERED' or o.displayStatus eq 'SUCCESS' or o.displayStatus eq 'RECEIVED'}">
                                                    <a class="order-action primary"
                                                        href="${pageContext.request.contextPath}/customer/returns?action=create&amp;orderId=${o.id}">Request
                                                        Return</a>
                                                </c:if>
                                        </div>
                                    </footer>
