                                                    ...
                                                    <c:forEach var="item" items="${productList}">
                                                        <tr>
                                                            <td class="ps-4">
                                                                ...
                                                                <span class="badge bg-indigo-subtle text-indigo border mt-1">${item.brandName} | ${item.color} - ${item.size}</span>
                                                            </td>
                                                            <td class="font-mono small fw-bold text-secondary">${item.sku}</td>
                                                            <td class="text-end text-muted"><fmt:formatNumber value="${item.costPrice}" pattern="#,##0"/> VND</td>
                                                            <td class="text-end fw-bold text-dark"><fmt:formatNumber value="${item.salePrice}" pattern="#,##0"/> VND</td>
                                                            ...
                                                            <td class="text-center">
                                                                <button class="btn btn-sm btn-outline-amber btn-edit" title="Update Details"
                                                                        data-bs-toggle="modal" data-bs-target="#editProductModal"
...
<%-- FORM EDIT DÙNG CHUNG (Mục 4: Admin sửa hết, Staff bị khóa các ô Giá bán lẻ) --%>
            ...
            <form action="${pageContext.request.contextPath}/admin/dashboard" method="POST" enctype="multipart/form-data">
                ...
                <div class="modal-body p-4">
                    <div class="row g-3">
                        ...
                        <div class="col-md-6 staff-readonly">
                        ...
                        <div class="col-md-6">
                            <label class="form-label fw-semibold text-danger">Listed Sale Price (VND) *</label>
                            <input type="number" class="form-control fw-bold" name="salePrice" id="edit_salePrice" min="0" required
                                   <%= !isAdmin ? "readonly style='background-color: #e9ecef;'" : "" %>>
                            <c:if test="<%= !isAdmin %>">
                                <small class="text-muted">(*) Tài khoản Staff không được quyền sửa đổi giá bán lẻ.</small>
                            </c:if>
                        </div>
    ...
    document.addEventListener("DOMContentLoaded", function () {
        ...
        document.querySelectorAll('.btn-edit').forEach(button => {
            button.addEventListener('click', function () {
                ...
                // Phục vụ phân luồng Staff
                const sku = this.getAttribute('data-sku') || "";
                document.getElementById('edit_sku_hidden').value = sku;
                document.getElementById('edit_sku_display').value = sku || "N/A (Chỉ cấu hình ở biến thể)";
                document.getElementById('edit_salePrice').value = parseInt(this.getAttribute('data-saleprice')) || 0;
                document.getElementById('edit_variantId').value = this.getAttribute('data-variantid') || "0";
            });
        });
    });

    // Toàn bộ phần vẽ đồ thị giữ nguyên 100%
    const now = new Date();
